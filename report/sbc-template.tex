...
\begin{resumo} 
    ...
    estados até encontrar a solução desejada. Entretanto, fazer uma busca exaustiva é
    ...
    relatório sera abordado quatro algorítimos de busca analisando um problema de achar
...
Com o sistema devidamente modelado, é então necessário percorrer esse grafo de estados em
...
para fazer essa busca diferentes estratégias foram elaboradas, como
...
Este relatório irá abordar essas quatro estratégias de busca, explicando o que cada algoritmo
...
\begin{figure}[h]
    ...
    \caption{Exemplo da busca BFS. Fonte: Extraído de \cite{Russell:2009:AIM:1671238}}
    \label{fig:bfs_ex}
\end{figure}
...
\begin{figure}[h]
    ...
    \caption{Exemplo da Busca de Custo Uniforme. Fonte: Extraído de \cite{Russell:2009:AIM:1671238}}
    \label{fig:ucs_ex}
\end{figure}
...
\begin{figure}[h]
    ...
    \caption{Exemplo da Busca com Aprofundamento Iterativo. Fonte: Extraído de \cite{Russell:2009:AIM:1671238}}
    \label{fig:ids_ex}
\end{figure}
...
aleatórios foram gerados e para esses 300 pontos todos os algorítimos foram executados, guardando
a quantidade de nós abertos e o custo do caminho em cada execução. Informações como média,
mediana e desvio padrão foram calculadas a partir desses dados para análise posterior.

Para o algoritmo A* foram feitos experimentações com o peso da função heurística. Como discutido
na seção~\ref{sec:astar}, quando atribuído um peso para essa função o resultado deixa de ser ótimo
e passa a abrir menos nós pois se aproxima mais de um algorítimo guloso. Foram testados quatro
diferentes pesos, 1 (sem superestimativa, resultado ótimo), 1.5, 3 e 5.
...
A tabela~\ref{tab:tab1} mostra a média e o desvio padrão para o número de nós abertos (expandidos) e
o custo do caminho. Inicialmente, vemos que os dois algoritmos ótimos, estes sendo o Custo Uniforme e
o A* sem superestimativa (peso 1.0) possuem os mesmos valores para o custo do caminho. O Algorítimo
de Custo Uniforme abriu aproximadamente 4 vezes mais nós que o A*, sendo muito mais custoso.

A Busca em Largura e o Aprofundamento Iterativo também possuem os mesmos valores para o custo do
caminho, já que ambos se assemelham e ambos acham o caminho do menor nível da árvore. Não é o caminho
ótimo nesse problema pois os nós possuem custos próprios. O Aprofundamento Iterativo abriu aproximadamente
5 vezes mais nós que a Busca em Largura devido as $n$ iterações do algoritmo. Novamente, o ponto forte
da busca com Aprofundamento Iterativo é a complexidade de memória, que é menor que o BFS.

Quanto ao A*, conforme o peso da função heurística foi aumentando, também aumentou-se o custo médio
do caminho, entretanto, o número de nós aberto diminuiu drasticamente. Isso ocorre devido ao comportamento
...
A Figura~\ref{fig:plot1} mostra 3 gráficos. O primeiro mostra a relação entre a média
do custo dos caminhos e os nós abertos e os dois abaixo mostram o Box plot da quantidade
de Nós Aberto e do Custo do Caminho respectivamente. Em relação ao primeiro gráfico, os
algoritmos que estão mais próximos da origem são os melhores. O A* com os 3 pesos iniciais
são os melhores, e a escolha de qual peso utilizar vai depender do que se deseja priorizar:
custo do caminho ou abertura de nós. A busca de Custo Uniforme, embora tenha produza o caminho
ótimo perde no quesito nós abertos para o A*, logo não deve ser considerado.

No Box plot de Nós Abertos, vemos que, estatisticamente o Custo Uniforme e o BFS são
muito parecidos. Também observa-se que a quantidade de nós abertos no A* cai drasticamente
conforme o peso da função heurística aumenta. Tal fato pode ser analisado com mais detalhes
na figura~\ref{fig:plot2}.

Já no Box plot do Custo do Caminho percebe-se
que estatisticamente os 3 primeiros testes do A* são bastantes similares. Em aplicações
em que o custo do caminho não é essencial e como o número
de nós cai mais drasticamente em relação ao custo do caminho, talvez seria interessante utilizar
um maior peso para o cálculo do caminho.
...
A escolha do algoritmo de busca depende principalmente do que você quer priorizar, logo uma 
analise detalhada do problema é de extrema importância para se chegar a um resultado
desejado. Os algoritmos estudados podem ser resumidos na tabela~\ref{tab:tab2}. A Busca
heurística apresentou um resultado muito melhor aos demais algoritmos, entretanto não
é todos os problemas que podem fazer uso desses métodos.

No algorítimo atual, não há espaços que não são atingíveis, logo, um possível
experimento para ser realizado futuramente é adicionar barreiras na qual não é possível
movimentar-se. É possível que o comportamento de algum dos algoritmos se altere quando
adicionada essa mudança.



\begin{table}[h]
    \caption{Resultados obtidos na execução de 300 pontos aleatórios}\label{tab:tab2}
    \centering
    \begin{tabular}{llll}
        \toprule
        Busca & Ótimo & Completo & Observações \\
            \midrule
        Busca em Largura            & Não & Sim & --    \\
        Custo Uniforme              & Sim & Sim & Útil quando não se conhece o destino    \\
        Aprofundamento Iterativo    & Não & Sim & Baixo uso de memória    \\
        A* ($h(x)$ com peso 1.0)    & Sim & Sim & --  \\
        A* ($h(x)$ com peso 1.5)    & Não & Sim & Superestimativa gera menos nós \\
        A* ($h(x)$ com peso 3.0)    & Não & Sim & Superestimativa gera menos nós \\
        A* ($h(x)$ com peso 5.0)    & Não & Sim & Superestimativa gera menos nós \\
        \bottomrule
  \end{tabular}
\end{table}
